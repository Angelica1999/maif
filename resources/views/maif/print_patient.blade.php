            ...
            <div style="margin-left:20px; margin-right:20px">
                @if($patient->barangay == null)
                    @if($patient->region == null)
                        <p>This is to certify that patient <strong>{{ $patient->fname . ' ' . $patient->mname . ' ' . $patient->lname}}</strong>,{{ ($age == 0) ? '' : ' ' . $age . ',' }} was extended by this office, a total of {{ number_format(str_replace(',','',$patient->guaranteed_amount), 2, '.', ',') }} medical assistance stated below based on the existing guidelines of Administrative Order No. 2025 - 0011, dated May 15, 2025:</p>
                    @else
                        <p>This is to certify that patient <strong>{{ $patient->fname . ' ' . $patient->mname . ' ' . $patient->lname}}</strong>,{{ ($age == 0) ? '' : ' ' . $age . ',' }} of {{$patient->other_barangay . ', '. $patient->other_muncity . ', '. $patient->other_province}}, was extended by this office, a total of {{ number_format(str_replace(',','',$patient->guaranteed_amount), 2, '.', ',') }} medical assistance stated below based on the existing guidelines of Administrative Order No. 2025 - 0011, dated May 15, 2025:</p>
                    @endif
                @else
                    <p>This is to certify that patient <strong>{{ $patient->fname . ' ' . $patient->mname . ' ' . $patient->lname}}</strong>,{{ ($age == 0) ? '' : ' ' . $age . ',' }} of {{$patient->barangay->description . ', '. $patient->muncity->description . ', '. $patient->province->description}}, was extended by this office, a total of {{ number_format(str_replace(',','',$patient->guaranteed_amount), 2, '.', ',') }} medical assistance stated below based on the existing guidelines of Administrative Order No. 2025 - 0011, dated May 15, 2025:</p>
                @endif
